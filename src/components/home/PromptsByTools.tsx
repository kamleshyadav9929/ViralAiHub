    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-border2 pb-4">
        <div className="space-y-1">
          <h2 className="font-heading text-xl sm:text-2xl font-bold text-textPrimary tracking-tight">
            Prompts by Tools
          </h2>
          <p className="text-[11px] sm:text-xs text-textSecondary font-light leading-none">
        ...
        {tools.map((tool) => {
          ...
          return (
            <motion.div
              ...
              className="bg-white border border-border1/60 hover:border-primary/20 rounded-2xl p-4 flex flex-col justify-between h-[132px] cursor-pointer shadow-[0px_1px_3px_rgba(0,0,0,0.01)] hover:shadow-[0px_10px_20px_-10px_rgba(0,0,0,0.05)] transition-all duration-300 relative group"
            >
              <div className="space-y-3">
                {/* Logo & Category Badge Row */}
                <div className="flex items-center justify-between">
                  {/* Tool Logo */}
                  <div className="w-9 h-9 rounded-xl overflow-hidden border border-border1/40 flex items-center justify-center p-1 bg-surface2 shadow-sm shrink-0">
                    {imgFallbackLevel < 2 ? (
                      <img
                        src={imgSrc}
                        alt={`${tool.name} Logo`}
                  ...
                  {/* Category Pill Icon */}
                  <div className={cn("w-6 h-6 rounded-full flex items-center justify-center border shrink-0", tool.bgClass)}>
                    <CategoryIcon size={12} className={tool.color} />
                  </div>
                </div>

                {/* Name & Domain info */}
                <div className="space-y-0.5 text-left">
                  <h4 className="text-xs font-semibold text-textPrimary group-hover:text-primary transition-colors duration-200 truncate">
                    {tool.name}
                  </h4>
                  <p className="text-[9px] text-textMuted font-mono truncate leading-none">
                    {info.domain}
                  </p>
                </div>
              </div>
              ...
              {/* Bottom Card Footer */}
              <div className="flex items-center justify-between pt-2 border-t border-border1/30">
                <span className="text-[9px] uppercase font-bold tracking-wider text-textMuted font-mono">
                  {tool.category}
                </span>
