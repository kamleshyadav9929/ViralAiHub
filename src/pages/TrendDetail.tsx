  ...
  if (error || !trend) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center space-y-4 text-center">
        <div className="bg-rose-500/10 text-rose-500 p-4 rounded-full border border-rose-500/20 mb-2">
          <AlertTriangle size={32} />
        </div>
        <h2 className="text-xl font-bold text-textPrimary">Trend Guide Not Found</h2>
  ...
  return (
    <>
      ...
      <div className="px-6 md:px-12 py-6 pt-16">
        {/* Breadcrumbs */}
        <div className="flex items-center space-x-2 text-xs text-textMuted mb-6">
        <Link to="/" className="hover:text-textPrimary transition-colors">Explore</Link>
        <ChevronRight size={10} />
        {trend.category && (
          <>
            <Link to={`/category/${trend.category.slug}`} className="hover:text-textPrimary transition-colors">
              {trend.category.name}
            </Link>
            <ChevronRight size={10} />
          </>
        )}
        <span className="text-textSecondary font-semibold truncate max-w-[200px]">
          {trend.title}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 mt-10">
              ...
              <div className="space-y-6">
                {trend.steps && trend.steps.length > 0 ? (
                  <>
                    ...
                    <Card hoverEffect={false} className="relative overflow-hidden bg-deepGreen border border-transparent p-8 rounded-2xl mt-8 text-white">
                      <div className="relative z-10 space-y-4 max-w-lg">
                        <div className="inline-flex items-center space-x-1.5 text-xs text-secondary font-mono tracking-widest font-bold uppercase">
                          <Sparkles size={14} />
                        ...
                        <Button variant="premium" size="sm" className="flex items-center gap-1.5 font-semibold text-xs mt-2">
                          <Download size={12} />
                          Get Premium Pack
                        </Button>
                      </div>
                  ...
                  <div className="text-center py-12 bg-surface2 border border-border1 rounded-2xl">
                    <p className="text-xs text-textSecondary">No steps defined for this guide yet.</p>
                  </div>
                )}
              </div>
            )}
        ...
        <div className="space-y-8">
          ...
          <Card hoverEffect={false} className="p-6 space-y-4">
            <h3 className="font-heading text-xs uppercase tracking-wider text-textPrimary font-bold flex items-center gap-2">
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex flex-col text-left">
                        <span className="font-semibold text-textPrimary text-xs leading-none mb-1">{tool}</span>
                        <span className="text-[10px] text-textMuted font-light leading-none">{info.domain}</span>
                      </div>
                    </div>
                    <ChevronRight size={12} className="text-textMuted group-hover/tool:text-primary transition-colors transform group-hover/tool:translate-x-0.5 duration-200" />
                  </a>
                );
              })}
            </div>
              ...
              <div className="flex flex-col space-y-4">
                {relatedTrends.map((rel) => (
                  <Link 
                    key={rel.id} 
                    to={`/trend/${rel.slug}`}
                    className="flex space-x-3 group/item border-b border-border1 last:border-b-0 pb-3 last:pb-0"
                  >
                    <div className="w-16 h-12 rounded-lg overflow-hidden bg-surface1 shrink-0 border border-border1">
                      <img 
                    ...
                    <div className="space-y-1">
                      <h4 className="text-xs font-medium text-textPrimary leading-tight group-hover/item:text-secondary transition-colors duration-200 line-clamp-2">
                        {rel.title}
                      </h4>
                      <div className="flex items-center space-x-2 text-[10px] text-textMuted">
                        ...
                        <span>•</span>
                        <span className="font-semibold text-actionBlue">{rel.difficulty}</span>
                      </div>
                    </div>
                  </Link>
                ))}
          ...
          {trend.prompts && trend.prompts.length > 0 && (
            <Card hoverEffect={false} className="p-6 space-y-4 bg-surface2 border border-border1">
